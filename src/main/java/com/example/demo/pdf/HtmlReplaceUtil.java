...
public class HtmlReplaceUtil {
    public Object replaceHtml(Object object)
            throws NoSuchMethodException, SecurityException, Exception {
        // 不需要的自己去掉即可
        if (object != null) {
            if (object instanceof String) {
                // 如果输入对象直接为String类型，则直接替换。
                object = HtmlRegexpUtil.replaceTag(object.toString())
                    .toString();
                return object;
            }
            ...
            for (int j = 0; j < fields.size(); j++) {
                ...
                try {
                    ...
                    if (field.getGenericType().toString()
                        .equals("class java.lang.String")) { // 如果type是类类型，则前面包含"class
                                                             // "，后面跟类名

                        // 拿到该属性的get方法
                        Method getM = (Method) object.getClass()
                            .getMethod("get" + getMethodName(field.getName()));
                        // 调用getter方法获取属性值
                        ...
                        // 拿到该属性的set方法
                        Method setM = (Method) object.getClass().getMethod(
                    ...
                    if (field.getGenericType().toString()
                        .indexOf("com.hs.zt.ism.modules.im") > -1) {
                        // 拿到该属性的get方法
                        Method getM = (Method) object.getClass()
                            .getMethod("get" + getMethodName(field.getName()));
                        // 调用getter方法获取属性值
                    ...
                    // 列表中对象替换
                    if (field.getGenericType().toString()
                        .indexOf("java.util.List") > -1
                            || field.getGenericType().toString()
                                .indexOf("java.util.ArrayList") > -1) {
                        // 拿到该属性的get方法
                        Method getM = (Method) object.getClass()
                            .getMethod("get" + getMethodName(field.getName()));
                        // 调用getter方法获取属性值
                    ...
                    // Map中对象替换
                    if (field.getGenericType().toString()
                        .indexOf("java.util.HashMap") > -1
                            || field.getGenericType().toString()
                                .indexOf("java.util.Hashtable") > -1) {
                        // 拿到该属性的get方法
                        Method getM = (Method) object.getClass()
                            .getMethod("get" + getMethodName(field.getName()));
                        // 调用getter方法获取属性值
